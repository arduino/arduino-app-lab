...
import { customInoAutocomplete } from '../autocomplete/langs/ino';
...
export const getKeywordsExtensions = ({
  keywords,
  isLspEnabled,
}: {
  keywords: KeywordMap;
  ...
  // autocomplete
  if (!isLspEnabled) {
    keywordsPlugins.push(customInoAutocomplete(keywords));
  }

  return keywordsPlugins;
};

const createKeywordsMatcher = (
  keywords: KeywordMap,
